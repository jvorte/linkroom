<x-app-layout>
    <div class="relative h-64 overflow-hidden shadow-lg mb-8">
        <img src="{{ asset('storage/images/prof4.jpg') }}" alt="Header background" class="absolute inset-0 w-full h-full object-cover brightness-75">
    ...
    <div class="max-w-6xl mx-auto px-4 py-6">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
            <form method="GET" action="{{ route('professionals.index') }}" class="w-full sm:w-auto flex-grow">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.search_placeholder') }}" oninput="if (this.value === '') this.form.submit()" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
            </form>

            <button type="button" onclick="document.getElementById('filterModal').classList.remove('hidden')" class="bg-slate-800 text-white px-4 py-2 rounded hover:bg-slate-700 flex items-center gap-2">
        ...
        <div id="filterModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
            <div class="bg-white w-full max-w-2xl mx-auto rounded-lg shadow-lg p-6 relative">
                <button class="absolute top-2 right-3 text-gray-500 hover:text-black text-2xl" onclick="document.getElementById('filterModal').classList.add('hidden')">&times;</button>
            ...
            <ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($professionals as $user)
                    <li class="relative border rounded-lg p-5 shadow-xl bg-white hover:shadow-md transition fade-in-up">
                        ...
                        <a href="{{ route('profile.show', $user->slug) }}" class="block text-xl font-semibold text-blue-700 hover:underline">
                            {{ $user->name }}
                        </a>
